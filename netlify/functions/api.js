...
function jsonResponse(statusCode, body) {
  return { statusCode: statusCode, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }, body: JSON.stringify(body) };
}
...
function proxyStream(url, method, token, portal, mac, cmd, transcode) {
  return new Promise(function(resolve, reject) {
...
exports.handler = async function(event, context) {
  ...
  if (path === '/api/status') {
    return jsonResponse(200, { ok: true, ffmpeg: !!getFfmpeg(), node: process.version });
  }
  ...
  if (path === '/api/stalker/channels' && method === 'POST') {
    ...
    try {
      ...
      if (resp.statusCode === 200) {
        return jsonResponse(200, JSON.parse(resp.data.toString()));
  ...
  if (path === '/fetch' && method === 'GET') {
    ...
    try {
      var resp = await proxyStream(target, 'GET', null, null, null, null, false);
